...
# ---------------------------------------------------------
# PRE-COMPILE all Python source to .pyc bytecode
# This makes every cold start skip Python parsing entirely.
# ComfyUI + all 6 custom nodes get compiled here once at build time.
# Saves 20-40s of Python startup cost on every cold start.
# ---------------------------------------------------------
RUN python -m compileall -q /comfyui /usr/lib/python3.12 \
    $(python -c "import site; print(' '.join(site.getsitepackages()))") \
    2>/dev/null || true

# ---------------------------------------------------------
# Bake models into image — eliminates all runtime downloads
# ---------------------------------------------------------
RUN mkdir -p \
    /comfyui/models/unet \
    /comfyui/models/vae \
